...
import { ArrowLeft, ArrowRight, Check, ChevronDown, CircleHelp, Lightbulb, List, MessageSquareText, Mic, Square } from 'lucide-react'
  ...
  return (
    <main className="flex h-full min-h-0 w-full min-w-0 overflow-hidden border-r border-line bg-white">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* 顶部 */}
        <div className="flex h-[48px] items-center justify-between border-b border-line px-6 flex-none">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="lg:hidden inline-flex h-[28px] items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-[12px] font-medium text-text-secondary"
            ...
            <div className="inline-flex items-center rounded-lg bg-surface-soft p-0.5 text-[12px] font-medium">
              <button
                type="button"
                onClick={() => { if (!strictMode) onToggleStrict() }}
                className={`h-[24px] rounded-[6px] px-2.5 transition-colors ${strictMode ? 'bg-white text-text-primary shadow-[0_1px_2px_rgba(16,24,40,0.08)]' : 'bg-transparent text-text-tertiary hover:text-text-secondary'}`}
                aria-pressed={strictMode}
                title="隐藏要点与提示，考察真实掌握度"
              >模拟测试</button>
              <button
                type="button"
                onClick={() => { if (strictMode) onToggleStrict() }}
                className={`h-[24px] rounded-[6px] px-2.5 transition-colors ${!strictMode ? 'bg-white text-text-primary shadow-[0_1px_2px_rgba(16,24,40,0.08)]' : 'bg-transparent text-text-tertiary hover:text-text-secondary'}`}
                aria-pressed={!strictMode}
                title="展示全部要点与建议回答，边看边学"
              >学习练习</button>
            </div>
            <span className="text-[12px] text-text-tertiary">{covered.length}/{totalPoints} 项已验证</span>
          </div>
        </div>
        <div className="h-[3px] bg-line flex-none"><span className="block h-full bg-brand transition-[width] duration-300" style={{ width: `${coverage}%` }} /></div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="mx-auto max-w-[680px]">
            ...
            {/* 历史问答 */}
            {!showAllTurns && turns.length > 2 && (
              <button
                type="button"
                onClick={() => setShowAllTurns(true)}
                className="mb-6 w-full rounded-lg border border-line bg-surface-soft px-4 py-2.5 text-[12px] font-medium text-text-tertiary hover:text-text-secondary transition-colors"
              >
                展开更早的 {turns.length - 2} 轮问答
              </button>
            )}
            {turns.map((turn, i) => {
              if (!showAllTurns && i < turns.length - 2) return null
              return (
              <div key={i} className="mb-6">
                ...
                {!strictMode && turn.intent && <p className="text-[12px] text-text-tertiary mb-4">{turn.intent}</p>}
                ...
                {turn.annotation && (
                  <div className="mt-2 flex items-start gap-2 rounded-lg border border-warning/25 bg-warning-soft px-3.5 py-2.5 text-[13px] leading-relaxed text-text-secondary">
                    ...
                    <span><strong className="font-semibold">没听懂：</strong>{turn.annotation}</span>
                  </div>
                )}
                {turn.action !== 'skip' && (!strictMode || !turn.answer) && turn.answerSuggestion && (
            ...
            {currentQuestion && !done && (
              <div ref={currentQuestionRef} className="mb-6 scroll-mt-4">
                <div className="rounded-xl border-2 border-brand bg-white px-5 py-4 shadow-[0_1px_6px_rgba(16,24,40,0.06)]">
                  ...
                  {currentIntent && !strictMode && <p className="mt-1 text-[13px] leading-relaxed text-text-secondary">{currentIntent}</p>}
                  {currentIntent && strictMode && (
                    <button
                      type="button"
                  ...
                  {currentIntent && strictMode && intentOpen && (
                    <p className="mt-1 text-[13px] leading-relaxed text-text-secondary">{currentIntent}</p>
                  )}
                </div>
              </div>
            )}
        ...
        {/* 底部输入 */}
        <div className="border-t border-line bg-white px-6 py-4 flex-none">
          <div className="mx-auto max-w-[680px]">
            {!done ? (
              <div className="flex flex-col gap-3">
                <textarea
                  className="w-full resize-none rounded-xl border border-line-strong bg-white px-4 py-3 text-[14px] leading-relaxed text-text-primary placeholder:text-text-tertiary focus:border-[#60a5fa] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.1)] min-h-[100px] max-h-[260px]"
                  value={answer}
