...
function InterviewerBubble({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2.5 max-w-[85%]">
      ...
      <div className="rounded-xl rounded-tl-[4px] bg-[#f0f3f5] px-3.5 py-2.5 text-[13px] leading-[1.55] text-[#30373c]">
        {children}
      </div>
    </div>
  )
...
function UserBubble({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[85%] rounded-xl rounded-tr-[4px] bg-brand px-3.5 py-2.5 text-[13px] leading-[1.55] text-white">
        {children}
      </div>
    </div>
  )
...
function SystemMsg({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex justify-center">
      <span className="rounded-full bg-[#eef1f2] px-3 py-1 text-faint text-[10px]">{children}</span>
    </div>
  )
}
  ...
  return (
    <main className="flex flex-col min-w-0 bg-canvas" style={{ maxHeight: 'calc(100vh - 54px)' }}>
      ...
      <div className="flex h-[44px] items-center justify-between border-b border-line bg-white px-5 flex-none">
        <button type="button" className="flex items-center gap-1.5 bg-transparent text-muted text-[11px] hover:text-ink" onClick={onBackToAudit}>
          <ArrowLeft size={14} />返回
        </button>
        <div className="flex items-center gap-2 text-center min-w-0">
          <strong className="text-[12px] text-[#1a2024] truncate max-w-[200px]">{selected.title}</strong>
          <span className="text-faint text-[10px] flex-none">· {roundLabel}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <div className="mx-auto max-w-[640px] flex flex-col gap-3">
          ...
          <div className="rounded-lg border border-line bg-white px-3.5 py-2.5 text-[12px] leading-[1.6] text-[#4f5960]">
            “{selected.quote}”
          </div>
            ...
            <div className="flex flex-col gap-1.5 pt-1">
              <div className="flex flex-wrap gap-1.5">
                {selected.evaluationPoints.map((point) => (
                  <span
                    key={point}
                    className={`rounded px-2 py-0.5 text-[10px] ${covered.includes(point) ? 'text-green bg-green-soft' : 'text-faint bg-[#eef1f2]'}`}
                  >
              ...
              {missing.length > 0 && (
                <p className="flex items-start gap-1 text-[10px] text-amber">
        ...
        <div className="mx-auto max-w-[640px]">
          {!done ? (
            <div className="flex gap-2">
              <div className="flex-1 flex flex-col gap-1.5">
                <textarea
                  className="w-full resize-none rounded-lg border border-line-strong bg-white px-3 py-2 text-[12px] leading-[1.6] text-[#283136] placeholder:text-faint min-h-[52px] max-h-[120px]"
                  value={answer}
              ...
              <div className="flex flex-col gap-1.5">
                <Button variant="primary" className="h-full px-4 flex-col gap-1" disabled={answer.trim().length < 8 || loading} onClick={onSubmit}>
                  ...
                  <span className="text-[10px]">{loading ? '…' : '发送'}</span>
                </Button>
          ...
          {showHint && !done && (
            <div className="mt-2 flex gap-2 rounded-md border border-[#ead5ae] bg-amber-soft p-2.5 text-[#72521e]">
              ...
              <div>
                <strong className="block text-[10px]">回答应覆盖</strong>
                <p className="m-0 mt-0.5 text-[10px] leading-[1.45]">{selected.evaluationPoints.join('、')}</p>
              </div>
            </div>
          )}
        </div>
