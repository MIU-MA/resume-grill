  ...
  return (
    <aside className="flex flex-col border-line bg-surface md2:min-h-[calc(100vh-58px)] md2:border-r max-md1:min-h-0 max-md1:border-b">
      <div className="flex items-center gap-[10px] border-b border-line px-[15px] py-[15px] pb-[13px] max-md1:hidden">
        <div className="grid size-[34px] place-items-center rounded-full flex-none bg-[#e7ecef] text-[#28343a] text-[10px] font-extrabold">{analysis.candidate.slice(0, 2)}</div>
        <div className="flex min-w-0 flex-col">
          <strong className="text-[12px]">{analysis.candidate}</strong>
          <span className="text-muted mt-[2px] text-[9px]">{analysis.role}</span>
        </div>
      </div>

      <div className="flex h-[38px] items-center justify-between px-[15px] text-muted text-[9px] font-750 uppercase max-md1:h-[34px]">
        <span>简历声明</span>
          ...
          return (
            ...
            >
              ...
              <span className="flex min-w-0 flex-col">
                <small className="text-faint mb-[2px] text-[8px]">{CLAIM_CATEGORY_LABELS[claim.category]}</small>
                <strong className={`overflow-hidden text-ellipsis whitespace-nowrap text-[10px] font-650 ${selectedIndex === index ? 'text-[#183b86]' : 'text-[#30373c]'}`}>{claim.title}</strong>
              </span>
              <span className={`justify-self-end font-mono text-[10px] font-bold leading-none ${RISK_SCORE[risk.color]}`}>{claim.askLikelihood}</span>
            </button>
          )
        })}
      </div>
      ...
      <div className="mt-auto flex gap-2 border-t border-line bg-[#f8f9fa] px-[14px] py-[13px] text-muted max-md1:hidden">
        ...
        <p className="m-0 text-[8px] leading-[1.55]"><strong className="text-[#505a60]">评分不是能力结论</strong><br />它表示这句话在面试中被继续追问的概率。</p>
      </div>
    </aside>
  )
}
