...
import { sourceUrlSchema, type CareerPage } from '@/domain/mail-schema'

export function LinkImporter({ existingUrls, slots, connected, onConnect, read, onImported, onBusy }: {
  ...
  const importLinks = async () => {
    ...
    const values = [...new Set(input.split(/\s+/).filter(Boolean))]
  ...
  return <section className="flex-none border-b border-line bg-white px-4 py-3 sm:px-5" aria-label="导入招聘链接">
    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1"><h2 className="m-0 text-[13px] font-semibold">粘贴招聘链接</h2><span className="text-[12px] text-text-tertiary">每行一个，自动整理岗位、招聘邮箱和邮件草稿</span></div>
    <div className="mt-2 flex items-end gap-3"><textarea aria-label="招聘链接，每行一个" className="mail-input h-[72px] flex-1 resize-y font-mono text-[12px]" placeholder="https://公司官网/招聘详情" disabled={busy} value={input} onChange={e => setInput(e.target.value)} /><Button loading={busy} disabled={!input.trim() || slots <= 0} onClick={() => void importLinks()}>{connected ? '整理到清单' : '连接后整理'}</Button></div>
