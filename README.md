...
- 可选粘贴目标岗位描述，比较岗位要求与简历证据的匹配程度
- 针对单条陈述进行 3–5 轮动态追问
- 跟踪已覆盖和仍缺失的评估要点
- 生成“回答结论、已证明证据、仍缺证据、下一步行动”、改写建议和 Markdown 汇总报告
...
项目不内置真实简历截图或真实岗位数据，避免 README 和测试素材泄露个人信息。发布前可以使用上述示例材料录制一段 20–30 秒的流程 GIF，放在本节顶部。
...
- 如果填写目标岗位描述，岗位描述也会随本次分析发送给所配置的模型供应商，并与分析结果一起保存在当前浏览器。
...
Resume Grill 专注于简历陈述的证据审计和压力测试。JD 匹配只是分析前的可选上下文，不扩展为投递管理、Offer 跟踪或完整简历编辑器，以保持项目边界清晰并降低个人维护成本。
