...
简历练习的流程为：**简历检查 → 选择经历 → 面试练习（准备内容 → 开始面试 → 面试复盘）**。侧栏将准备、问答和复盘合并为一个「面试练习」入口；准备页查看经历、回答要点和薄弱处，面试页点击「开始问答」后才开始，复盘页查看已有结果。页面切换本身不启动新的面试，在同一经历的准备页与问答页间切换保留当前回答。已配置模型时，导入会自动使用当前模型检查一次简历；提取的完整简历文本会经同源后端发送至模型服务商，并可能产生调用费用。分析时原始文件不上传。邮箱投递另选简历原文件，经本机执行器作为附件发给已确认的招聘邮箱。
